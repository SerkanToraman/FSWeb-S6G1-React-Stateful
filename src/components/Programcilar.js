...
export default function Programcilar() {
  ...
  const [öneÇıkanlar,setÖneÇıkanlar]= useState(null);
  ...
  ////////////////deneme
  /*const sahne = programcilar.find((p) => p.id === öneÇıkanlar)
  console.log(sahne)
  return sahne.isim*/

    let isim = "";
    programcilar.forEach((item) => {
    if (item.id === öneÇıkanlar) {
      isim = item.isim;
    }
  });
  return isim;
  ...
  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: öneÇıkanlar===null ?'royalblue':'gold', // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          programcilar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {/* burada dev.id 'yi öne çıkan id'ye atayın */setÖneÇıkanlar(dev.id) }}>Kutla</button>
            </div>
          )
        }
      </div>
